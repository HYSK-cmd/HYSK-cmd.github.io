/* work.js
   ...
   appear in the carousel:
     ...
     kTop   the scene's own measured vertical extent, top and bottom, in
     kBot   units of the projector's scale and relative to the canvas
            centre. core.js frameTo() solves these for the spread and offY
            that drop the scene into whatever vertical band is free, so it
            is never clipped and never runs under the copy at any viewport
            size. Re-measure with tools/extent if a scene's geometry
            changes; do not hand-tune them.
     poseT  the second in this scene's own loop where it explains itself
            best. Every scene is drawn at t + poseT, which does three jobs
            with one number: the card opens on that frame instead of a
            random one, clicking the card restarts the story there, and
            under prefers-reduced-motion (where t never advances) the scene
            freezes there rather than at t=0, which for most of them is the
            emptiest instant in the loop. Tune by eye.
  ...
  const WORK = [
    {
      ...
      hue: "#8ab4f8",
      draw: glyphAgents, yaw: 0.62, pitch: 0.22, kTop: -0.498, kBot: 0.31, poseT: 8.0,
    ...
    },
    {
      ...
      hue: "#8ab4f8",
      draw: glyphHub, yaw: 0.55, pitch: -0.22, kTop: -0.229, kBot: 0.413, poseT: 3.3,
    ...
    },
    {
      ...
      hue: "#5ad1c4",
      draw: glyphTrajectory, yaw: 0.70, pitch: 0.30, kTop: -0.316, kBot: 0.402, poseT: 2.9,
    ...
    },
    {
      ...
      hue: "#e878b4",
      draw: glyphCamera, yaw: 0.52, pitch: -0.36, kTop: -0.239, kBot: 0.624, poseT: 1.2,
    ...
    },
    {
      ...
      hue: "#a48cf0",
      draw: glyphSpider, yaw: 0.60, pitch: -0.34, kTop: -0.008, kBot: 0.878, poseT: 4.5,
    ...
    },
    {
      ...
      hue: "#f0954a",
      draw: glyphTars, yaw: 0.58, pitch: -0.42, kTop: -0.213, kBot: 0.572, poseT: 5.4,
    ...
    },
    {
      ...
      // teal, not the palette's yellow: yellow sits right on top of C.b amber,
      // which is every scene's "happening now" colour, so the whole scene read
      // as one flat wash with no active state visible
      hue: "#5ad1c4",
      draw: glyphTodo, yaw: 0.50, pitch: 0.12, kTop: -0.445, kBot: 0.427, poseT: 7.2,
    ...
    },
    {
      ...
      hue: "#f0954a",
      draw: glyphDrone, yaw: 0.62, pitch: -0.30, kTop: -0.337, kBot: 0.806, poseT: 7.0,
